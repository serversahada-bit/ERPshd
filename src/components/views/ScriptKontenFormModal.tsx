...
import { generateNamaKonten, RAW_FIELD_SECTIONS, RAW_FIELDS, ScriptKontenRow } from '@/lib/scriptKonten';
import { MetaAdsProduct } from '@/lib/metaAdsProducts';

function buildInitialFormState(initialData: ScriptKontenRow | null): Record<string, string> {
...
interface ScriptKontenFormModalProps {
  mode: 'create' | 'edit';
  products: MetaAdsProduct[];
  initialData: ScriptKontenRow | null;
  onClose: () => void;
  onSaved: () => void;
}

export default function ScriptKontenFormModal({ mode, products, initialData, onClose, onSaved }: ScriptKontenFormModalProps) {
  ...
  const namaKonten = generateNamaKonten({
    format: form.format,
    funnel: form.funnel,
    kategori: form.kategori,
    stageAwareness: form.stageAwareness,
    angle: form.angle,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const payload: Record<string, string | number> = { productId: Number(productId), ...form, namaKonten };

    try {
      const url = mode === 'create' ? '/api/script-konten' : `/api/script-konten/${initialData?.id}`;
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b border-slate-100 shrink-0">
          <h3 className="font-bold text-sm sm:text-base text-slate-900">
            {mode === 'create' ? 'Tambah Data Script & Konten' : 'Edit Data Script & Konten'}
          </h3>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="flex-1 overflow-y-auto px-5 py-4 space-y-5">
          ...
          {RAW_FIELD_SECTIONS.map((section) => (
            <div key={section.title}>
              <div className="text-[10px] font-bold text-slate-400 uppercase tracking-wide mb-2">{section.title}</div>
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
                {section.fields.map((field) => (
                  <div key={field.key} className={field.type === 'textarea' ? 'col-span-2 sm:col-span-3' : ''}>
                    ...
                    {field.key === 'namaKonten' ? (
                      <>
                        <input
                          ...
                          aria-describedby="nama-konten-help"
                          readOnly
                          value={namaKonten}
                          placeholder="Otomatis setelah data lengkap"
                          className="w-full rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1.5 text-xs text-slate-700 focus:outline-none focus:ring-2 focus:ring-rose-500/40"
                        />
                        <p id="nama-konten-help" className="mt-1 text-[10px] text-slate-500">
                          Otomatis dari Format, Funnel, Kategori, Stage Awareness, Angle, Creator, dan Tanggal Order.
                        </p>
                      </>
                    ) : field.type === 'textarea' ? (
                      <textarea
