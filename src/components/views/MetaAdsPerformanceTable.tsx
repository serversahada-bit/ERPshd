...
import { Plus, Pencil, Trash2, Eye, Loader2, Inbox, ChevronsUpDown, ChevronUp, ChevronDown, ChevronRight, Download, Sparkles } from 'lucide-react';
import {
  MetaAdsFullRow,
  MetaAdsRawRecord,
  formatDisplayValue,
...
export default function MetaAdsPerformanceTable() {
  ...
  const [modalState, setModalState] = useState<{
    mode: 'create' | 'edit';
    data: (MetaAdsRawRecord & { id?: number }) | null;
    ...
    <div className="space-y-5">
      <div className="bg-white rounded-2xl border border-slate-200 shadow-2xs p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 shrink-0 flex-wrap">
          ...
          <button
            onClick={() => setModalState({ mode: 'create', data: null, autoFetchFromMeta: true })}
            className="px-3.5 py-2 rounded-xl border border-blue-200 bg-blue-50 hover:bg-blue-100 text-blue-700 font-semibold text-xs flex items-center gap-1.5 transition-colors"
          >
            <Sparkles size={15} />
            <span>Tambah Data dari Meta</span>
          </button>
          <button
      ...
      {modalState && (
        <MetaAdsFormModal
          ...
          autoFetchFromMeta={modalState.autoFetchFromMeta}
